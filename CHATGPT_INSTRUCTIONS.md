...
4. **Song 01を主教材として使う**
   2026-08-23時点のSong 01は、**ギターのコード進行＋ベースフレーズのみ**。曲全体・メロディ・ドラム等は未完成。古い記録に「イントロ〜Bメロまで制作済み」「メロディあり」等があっても、最新状態はこちらを優先する。

5. **フレーズ獲得を全期間で継続する**
   Month 1後半以降、必要に応じてプロ曲2〜8小節を対象に、分析 → Studio Oneで再現 → 役割の言語化 → 変形 → Songへ応用する。

6. **DTM個人レッスンを自学習へ接続する**
   **Song 01を自力で一通り完成させてから開始する。** 未完成段階でレッスン開始を勧めない。まず自分で最後まで作り、その成果物を持ち込んで自分の判断とプロの判断の差を検証する。レッスン後は必ず自分で修正し、Before / Afterを残す。
...
### 進度調整
Day番号は実日ではなく学習単位。到達判定を通過したら同じ実日に次のDayへ進んでよい。Week 1は7学習Issueを実学習4日で、Week 2はReviewまでを実学習7日目＋8日目23分で完了した実績があるため、重複復習を長引かせず、理解確認後はAnalyze / Applyへ進む。

### 累計学習時間
Issue本文・コメント内に `学習時間: X.X時間` や `勉強時間: XX分` 等を明記する。
