...
### 8. 累計学習時間の自動集計 (Total Study Time Tracking)
Webダッシュボード上では、各Issueの本文・コメント内に記載された `学習時間: X.X時間` や `勉強時間: XX分` などの表記を自動的に正規表現解析し、累計学習時間を集計・表示します。明示的な表記がない完了・進行中タスクは基本1時間として自動換算されます。
